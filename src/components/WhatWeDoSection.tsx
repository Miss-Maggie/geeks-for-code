...
const WhatWeDoSection = () => (
  <section className="my-16">
    <h2 className="text-3xl font-semibold text-primary dark:text-white text-center mb-4">
      What We Do
    </h2>
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
      {skills.map((skill) => (
        <div
          key={skill.title}
          className="bg-white dark:bg-[#232437] rounded-xl shadow-md p-8 flex flex-col items-center border border-gray-100 dark:border-[#323247] hover:shadow-lg transition-shadow animate-scale-in"
        >
          <div className="text-primary dark:text-white">{skill.icon}</div>
          <h3 className="mt-4 text-xl font-bold text-primary dark:text-white">{skill.title}</h3>
