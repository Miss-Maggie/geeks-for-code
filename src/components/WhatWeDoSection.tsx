...
const WhatWeDoSection = () => (
  <section className="my-14 xs:my-16 px-2 xs:px-0">
    <h2 className="text-2xl xs:text-3xl font-semibold text-primary dark:text-white text-center mb-4">
      What We Do
    </h2>
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 xs:gap-8">
      {skills.map((skill) => (
        <div
          key={skill.title}
          className="bg-white dark:bg-[#232437] rounded-xl shadow-md p-6 xs:p-8 flex flex-col items-center border border-gray-100 dark:border-[#323247] hover:shadow-lg transition-shadow animate-scale-in"
        >
          <div className="text-primary dark:text-white">{skill.icon}</div>
          <h3 className="mt-4 text-lg xs:text-xl font-bold text-primary dark:text-white">{skill.title}</h3>
          <p className="text-muted-foreground mt-2 text-center text-sm xs:text-base">{skill.desc}</p>
        </div>
      ))}
    </div>
  </section>
